...
ProgramWorker::ProgramWorker(Devices *device, FlashAlgo *algo)
{
    this->dev = device;
    this->algo = algo;
...
void ProgramWorker::read_chip(QByteArray *data)
{
    if (data == NULL)
    {
        qDebug("[ProgramWorker] read_chip fail data buf is nullptr");
    ...
    qDebug("[ProgramWorker] read_chip");
    ...
    // err = dev->chip_read_memory(flash_addr, (uint8_t *)data->data(), flash_size);
    if (err < 0)
    {
...
int32_t ProgramWorker::read(uint32_t addr, uint32_t size, QByteArray *data, uint32_t page_size)
{
    ...
    for (uint32_t i = 0; i < size; i += page_size)
    {
        ...
        err = dev->chip_read_memory(addr + i, (uint8_t *)(data->data()) + i, pr_size);
        if (err < 0)
        {
...
void ProgramWorker::write(uint32_t addr, QByteArray *data)
{
    ...
    for (uint32_t i = 0; i < fw_size; i += page_size)
    {
        ...
        err = dev->chip_syscall_exec(&sys_call_s, entry, addr + i, pw_size, program_buff, 0);
        if (err < 0)
        {
...
void ProgramWorker::verify(uint32_t addr, QByteArray *data)
{
    ...
    for (uint32_t i = 0; i < data->length(); i += page_size)
    {
        ...
        err = dev->chip_read_memory(addr + i, (uint8_t *)(tmp_buf.data()), pr_size);
        if (err < 0)
        {
            qDebug("[ProgramWorker] Read fail");
