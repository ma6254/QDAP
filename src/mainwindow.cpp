    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    {
        log_info(QString("firmware_file_path: %1").arg(firmware_file_path));
    }

    dialog_chip_selecter = new ChipSelecter(this);

    bool ok;
    ok = dialog_chip_selecter->switch_chip(chip_vendor_name, chip_series_name, chip_name);
    if (ok == false)
    {
        QMessageBox msgBox;
...
void MainWindow::config_save(void)
{
    QFile file;
    file.setFileName(config_file_path);
    if (file.exists())
...
int MainWindow::config_load(void)
{
    QFile file;
...
void MainWindow::cb_action_chip_select(void)
{
    ...
    chip_vendor_name = dialog_chip_selecter->vendor_name();
    chip_series_name = dialog_chip_selecter->series_name();
    chip_name = dialog_chip_selecter->chip_name();
